...
public class ServerConnection {
    ...
    public void goOnline(int port) {
        ...
        assert isConnected();
        String q = "INSERT INTO user (nick, ip, online, port) values ('" + localNick
        ...
        try {
            ...
            try {
                if (getIpForNick(localNick) != null) { // if nick collision
                    q = "UPDATE user set ip=SUBSTRING_INDEX(USER(),'@',-1), online=1, port=" + port + " WHERE nick='"
                            + localNick + "';";
                    nick_collision = true;
                } else { // if address collision
                    q = "UPDATE user set nick='" + localNick
                            + "', online=1 WHERE ip=SUBSTRING_INDEX(USER(),'@',-1) AND port=" + port + ";";
    ...
    public void goOffline() {
        ...
        assert isConnected();
        try {
            st.execute("INSERT INTO user (nick, ip, online) values ('" + localNick
